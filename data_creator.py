...
def create_dataset(path):
    ...
    for i in range(len(prot_flag)):
        data.append( prot_flag[i] + packets_in[i] + packets_out[i] + packet_in_bytes[i] + packet_out_bytes[i] + src_port[i] + dst_port[i] + tcp_flags[i] + icmp_code_flags[i] + duration[i] +  [src_AS[i], dst_AS[i]] )####[subnet_dstport[src_ip[i][:11], dst_ip[i], prot_num[i], dst_port_num[i]]] + 
####[ip_dstport[src_ip[i], dst_ip[i], prot_num[i], dst_port_num[i]]] + [ip_dstport_ret[src_ip[i], dst_ip[i], prot_num[i], dst_port_num[i]]] +
...
def create_block_input(sample_length, sequence_length, check_icmp, check_tcp, check_udp, check_ps, check_lg):
    ...
    write_to_file("./data/data.txt", inp, samples, seq_len)
    write_to_file("./data/labels.txt", output,  samples, 1)
